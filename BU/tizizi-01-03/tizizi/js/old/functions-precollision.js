...
function collision(){
	setInterval(function() {
		...
		*/
				for (var propA = 0 in mesAbricots[j]) {
					$("#console").append("<div>"+ propA + " : " + mesAbricots[j][propA] +"</div>");
					console.log(propA + " : " + mesAbricots[j][propA]);
				}
			}
		...
		/*
			...
			console.log("j[id] : "+ mesAbricots[j]["id"]);    // j[id] : peach-4
		*/
				for (var propT = 0 in mesTirs[j]) {
					$("#console-tir").append("<div>"+ propT + " : " + mesTirs[j][propT] +"</div>");

					console.log(propT + " : " + mesTirs[j][propT]);
				}
			}
		}
